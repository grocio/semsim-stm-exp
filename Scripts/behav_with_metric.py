"""
This script integrates (a) behavioral data, (b) lexico-semantic variables, and (c) semantic simialrity metric.
It creates a single csv file 'data_for_analysis.csv' with these variables.
"""
...
for listID_N in sorted(list(set(materials_df['listID']))):
    ...
    current_words_df = pd.concat([current_words_df,
        pd.DataFrame(index=[], columns=['euc_dist_from_cent'])])
    ...
    for i in range(len(current_words_df)):
        ...
        current_words_df.iat[i,colnames.index('euc_dist_from_cent')]\
                = distance.euclidean(word_vad, centroid)
...
df_for_analysis.to_csv('../Data/data_for_analysis.csv')
