# Exloratory anaysis with Chebyshev distance

library(lme4)
library(optimx)
library(dplyr)

data <- read.csv('../Data/data_for_exploratory_analysis.csv',
                      row.names=1)
...
# [7] "position"             "cheby_dist_from_cent" "v"
# [10] "a"                    "d"                    "imageability"
...
# Correct-in-position scoring
  if(1){
      ...
      sink("../Results/CHEBY_corret_in_position_results.txt")
      print(summary(model))
      sink()
  }
...
# Item correct scoring
  if(1){
      model <- glmer(item_correct ~ position +
                     ...
                     control = glmerControl(optimizer = "bobyqa", calc.derivs = TRUE),
                     data=data)
      sink("../Results/CHEBY_item_correct_results.txt")
      print(summary(model))
      sink()
  }
...
# Given that order errors are not observed unless words are recalled, the absolute number of order errors is meaningful if semantic similarity does not affect item correct.
  ...
  if(1){
      ...
      sink("../Results/CHEBY_order_errors_results.txt")
      print(summary(model))
      sink()
  }
}
